    ...
    private func showHUD(for keyType: NXKeyType, command: Bool) {
        Task { @MainActor in
            ...
            case .soundUp, .soundDown, .mute:
                ...
                NikiNotchCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(v))
            case .brightnessUp, .brightnessDown:
                if command {
                    let v = KeyboardBacklightManager.shared.rawBrightness
                    NikiNotchCoordinator.shared.toggleSneakPeek(status: true, type: .backlight, value: CGFloat(v))
                } else {
                    let v = BrightnessManager.shared.rawBrightness
                    NikiNotchCoordinator.shared.toggleSneakPeek(status: true, type: .brightness, value: CGFloat(v))
                }
