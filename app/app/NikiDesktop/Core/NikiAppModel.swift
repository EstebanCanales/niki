...
final class NikiAppModel: NSObject, ObservableObject, AVAudioRecorderDelegate, @preconcurrency AVAudioPlayerDelegate {
    ...
    private let notchOpenSettingsNotification = Notification.Name("com.niki.notch.openSettings")
    private let notchRestartNotification = Notification.Name("com.niki.notch.restart")
    private let notchOpenNotification = Notification.Name("com.niki.notch.open")
    private let notchCloseNotification = Notification.Name("com.niki.notch.close")
    ...
    func syncNotchBridge() {
        writeNotchBridgeConfig()
    }

    func ensureNotchRunning() {
        guard !didRequestNotchLaunch else { return }
        didRequestNotchLaunch = true
        openNotchApp()
    }

    func openNotchApp() {
        writeNotchBridgeConfig()
        if let app = runningNotchApp() {
            app.activate(options: [.activateAllWindows])
            notchBridgeStatus = "NikiNotch is running."
            return
        }

        guard let appURL = notchApplicationURL() else {
            notchBridgeStatus = "NikiNotch app not found."
            return
        }
        ...
        NSWorkspace.shared.openApplication(at: appURL, configuration: .init()) { [weak self] app, error in
            Task { @MainActor in
                if let error {
                    self?.notchBridgeStatus = "Open failed: \(error.localizedDescription)"
                } else if app != nil {
                    self?.notchBridgeStatus = "NikiNotch opened."
                } else {
                    self?.notchBridgeStatus = "NikiNotch open requested."
                }
            }
        }
    }

    func terminateNotchApp() {
        guard let app = runningNotchApp() else {
            notchBridgeStatus = "NikiNotch is not running."
            return
        }
        if app.terminate() {
            notchBridgeStatus = "NikiNotch closing."
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) { [weak app] in
                guard let app, !app.isTerminated else { return }
                app.forceTerminate()
            }
        } else {
            app.forceTerminate()
            notchBridgeStatus = "NikiNotch force closed."
        }
    }
    ...
    func openNotchSettings() {
        writeNotchBridgeConfig()
        openNotchApp()
        postNotchNotification(notchOpenSettingsNotification, delay: 0.45)
        notchBridgeStatus = "Notch settings requested."
    }

    func restartNotchApp() {
        writeNotchBridgeConfig()
        if runningNotchApp() != nil {
            DistributedNotificationCenter.default().postNotificationName(
                notchRestartNotification,
                object: nil,
                userInfo: nil,
                deliverImmediately: true
            )
            notchBridgeStatus = "NikiNotch restart requested."
            return
        }
    ...
    func setNotchVisible(_ visible: Bool) {
        ...
        if visible {
            openNotchApp()
            postNotchNotification(notchOpenNotification, delay: 0.35)
            notchBridgeStatus = "NikiNotch open requested."
        } else {
            postNotchNotification(notchCloseNotification)
            notchBridgeStatus = "NikiNotch close requested."
        }
    }
    ...
    private func loadPersistedState() {
        if let data = UserDefaults.standard.data(forKey: configKey),
           let config = try? JSONDecoder().decode(NikiStoredConfig.self, from: data) {
            ...
            if let storedPersona = config.personaProfile {
                personaProfile = storedPersona
            }
        }
    ...
    private func writeNotchBridgeConfig() {
        let directory = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".niki", isDirectory: true)
        let fileURL = directory.appendingPathComponent("notch-config.json")
        let config = NikiNotchBridgeConfig(
            baseUrl: backendBaseURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "http://127.0.0.1:8000" : backendBaseURL,
            apiKey: backendAPIKey,
            userId: userID.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "user-demo" : userID
        )
        ...
        do {
            ...
            try data.write(to: fileURL, options: .atomic)
            notchBridgeStatus = "Notch linked: \(config.baseUrl)"
        } catch {
            notchBridgeStatus = "Notch link failed: \(error.localizedDescription)"
        }
    }

    private func runningNotchApp() -> NSRunningApplication? {
        NSRunningApplication.runningApplications(withBundleIdentifier: notchBundleID).first
    }

    private func postNotchNotification(_ name: Notification.Name, delay: TimeInterval = 0) {
    ...
    private func notchApplicationURL() -> URL? {
        if let registered = NSWorkspace.shared.urlForApplication(withBundleIdentifier: notchBundleID) {
            return registered
        }
        ...
        let repoRoot = fileURL
            ...
            .deletingLastPathComponent()
        let candidates = [
            repoRoot.appendingPathComponent("NikiNotch/build/Debug/NikiNotch.app"),
            URL(fileURLWithPath: "/private/tmp/niki-notch-dd/Build/Products/Debug/NikiNotch.app")
        ]
